...
const htmlFiles = ["index.html", "methodology.html", "privacy.html", "404.html", "updates.html", "thank-you.html"];
const duplicateIds = htmlFiles.flatMap((file) => {
  const ids = [...contents[file].matchAll(/\bid="([^"]+)"/g)].map((match) => match[1]);
  return ids.filter((id, index) => ids.indexOf(id) !== index).map((id) => `${file}#${id}`);
});

const checks = [
  ["each HTML file contains one document", htmlFiles.every((file) => (contents[file].match(/<!doctype html>/gi) || []).length === 1)],
  ["HTML ids are unique within each page", duplicateIds.length === 0],
  ["sitemap contains one XML document", (contents["sitemap.xml"].match(/<\?xml/g) || []).length === 1 && (contents["sitemap.xml"].match(/<urlset\b/g) || []).length === 1],
  ["homepage has one H1", (contents["index.html"].match(/<h1[ >]/g) || []).length === 1],
  ["homepage identifies final Communique No. 333 status", contents["index.html"].includes("Communique No. 333 was published on 4 July")],
  ["homepage links official law", contents["index.html"].includes("resmigazete.gov.tr")],
  ["homepage links final guidance", contents["index.html"].includes("18379_gelir_vergisi_genel_tebligi_seri_no")],
  ["homepage links the official full guidance text", contents["index.html"].includes("/teblig/11874")],
  ...
  ["LLM context links primary sources", contents["llms.txt"].includes("Official Gazette") && contents["llms.txt"].includes("final Communique No. 333")],
  ...
  ["updates page has dated source-based entries", contents["updates.html"].includes('id="2026-07-04"') && contents["updates.html"].includes('id="2026-06-04"') && contents["updates.html"].includes("resmigazete.gov.tr") && contents["updates.html"].includes("gib.gov.tr")],
